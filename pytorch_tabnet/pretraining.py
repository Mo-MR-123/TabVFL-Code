...
class TabNetPretrainer(TabModel):
    ...
    def fit(
        ...
        # dataloaders are shuffled when weights=0, 
        # if wights=1 then sampler is used so no shuffling is then necessary
    ...
    def _train_epoch(self, train_loader):
        ...
        wasted_extra_func_call_time = 0

        for batch_idx, X in enumerate(train_loader):
            start_wasted_time = time.perf_counter()
            self._callback_container.on_batch_begin(batch_idx)
            wasted_extra_func_call_time += time.perf_counter() - start_wasted_time
        ...
        real_epoch_time = (end_epoch_time - start_epoch_time) - wasted_extra_func_call_time
        self.epoch_training_times.append(real_epoch_time)
        print(f"Epoch Time: {real_epoch_time:.3f}")
